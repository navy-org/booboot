...
#else
#define HANDOVER_KERNEL_BASE 0xc0000000
#define HANDOVER_UPPER_HALF 0xc0000000
#endif //! INTPTR_MAX
...
#define HANDOVER_REQ_START {.tag = HANDOVER_MAGIC}

#define HANDOVER_REQ_END {.tag = HANDOVER_END}

#define WITH_FB {.tag = HANDOVER_FB}

#define WITH_ACPI {.tag = HANDOVER_RSDP}

#define WITH_FDT {.tag = HANDOVER_FDT}

#define WITH_FILES {.tag = HANDOVER_FILE}

#define WITH_CMDLINE {.tag = HANDOVER_CMDLINE}
...
#define handover_foreach_record(h, r)                                          \
  if ((h)->count > 0)                                                          \
    for (size_t i = 0; i < (h)->count && (((r) = (h)->records[i]), 1); ++i)
...
void handover_insert(HandoverPayload *payload, size_t index,
